...
if __name__ == "__main__":
    ...
    out = os.path.expanduser("~/ml/latentmpc"); os.makedirs(out, exist_ok=True)
    print("3) evaluating ...")
    rnd  = evaluate(env, enc, pred, head, policy="random")
    plan_sr = evaluate(env, enc, pred, head, policy="plan", gif=os.path.join(out, "demo.gif"))
    print("4) rendering imagined-vs-actual ..."); imagine(env, enc, pred, dec, gif=os.path.join(out, "imagine.gif"))
    ...
    print(f"  demo.gif (reaching) + imagine.gif (left=ACTUAL, right=IMAGINED) in {out}")
